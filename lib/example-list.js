...
module.exports = class ExampleList {
  ...
  // An element, not a DocumentFragment: the kernel caches the Row descriptor and
  // re-appends `label` on every render, and appending a fragment empties it, so a
  // second paint of the same cached row would draw blank.
  label(item, offsets) {
    ...
    label.append(tag, highlight(item.title, offsets));
    return label;
  }
