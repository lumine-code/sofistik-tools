const { CompositeDisposable, Disposable, Emitter, Range, Point } = require("lumine");
...
module.exports = class TextView {
  constructor(defaultText, select, iconClass, prompt) {
    ...
    // text-error is the editor's own colour for this; error-message has never
    // had a rule anywhere, so the message rendered as ordinary body text.
    this.errorMessage.classList.add("error-message", "text-error");
