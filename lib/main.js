...
module.exports = {
  ...
  // command opens matches the completion context.
  ...
  currentHelp(mode, version) {
    ...
    editor.backwardsScanInBufferRange(/^ *[+\-$]?prog +(\w+)/i, range, (object) => {
      found = true;
      // The command catalogue keys the modules `WING` and `RESULTS`, but their
      // manuals are named after the programs. Only the file name is aliased;
      // the catalogue lookup in `inViewer` still wants what was written.
  ...
  inViewer(object, range, filePath, prog, reuse) {
    ...
    let dest = null;
    const editor = lumine.workspace.getActiveTextEditor();
    const keywordContext = this.environmentProvider?.getKeywordContext?.({ editor });
