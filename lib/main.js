const { Disposable, CompositeDisposable, Point, BufferedProcess } = require("lumine");
...
const PACKAGE_LABEL = "SOFiSTiK Tools";

const COMMAND_LABELS = new Map([
  ["calculation-wps", "Calculation WPS"],
  ["calculation-wps-immediately", "Calculation WPS Immediately"],
  ["calculation-wps-current", "Calculation WPS Current"],
  ["calculation-sps", "Calculation SPS"],
  ["export-cdb", "Export CDB"],
  ["export-plb-to-docx", "Export PLB To DOCX"],
  ["clear-urs-tags", "Clear URS Tags"],
  ["open-wingraf", "Open WinGRAF"],
  ["open-dbinfo", "Open DBInfo"],
  ["open-ssd", "Open SSD"],
  ["open-sofiplus", "Open SOFiPLUS"],
  ["open-cdbase", "Open CDBASE"],
]);

/**
 * Attaches the display name for a command whose label carries a SOFiSTiK word,
 * and leaves every other command to the humanizer.
...
function labelled(name, didDispatch) {
  const half = COMMAND_LABELS.get(name.split(":")[1]);
  if (!half) return didDispatch;
  return { displayName: `${PACKAGE_LABEL}: ${half}`, didDispatch };
}

/**
 * The same, over a whole command map, for the groups that need no wrapper of
 * their own.
 * @param {Object} commands - command name to handler
 * @returns {Object} a command map for `lumine.commands.add`
 */
function withLabels(commands) {
  return Object.fromEntries(
    Object.entries(commands).map(([name, run]) => [name, labelled(name, run)]),
  );
}
...
module.exports = {
  ...
  activate() {
    ...
    // Three groups. The first needs a SOFiSTiK editor, the third needs a
    // tree-view selection, and the middle one exists in both flavours — those
    // used to be told apart by the registration selector, but both selectors
    // sit inside the workspace now, so one registration picks by where the
    // dispatch came from. Keeping both would run both, since handleCommandEvent
    // runs every matching listener on the way up.
    const inEditor = (commands) =>
      Object.fromEntries(
        Object.entries(commands).map(([name, run]) => [
          name,
          labelled(name, () => {
            if (this.sofistikEditor()) run();
          }),
        ]),
      );

    const bySurface = (pairs) =>
      Object.fromEntries(
        Object.entries(pairs).map(([name, { tree, editor }]) => [
          name,
          labelled(name, (event) => {
            if (event?.target?.closest?.(".tree-view")) return tree();
            if (this.sofistikEditor()) return editor();
          }),
        ]),
      );
    ...
    this.disposables.add(
      lumine.commands.add(
        ...
        inEditor({
          ...
          "sofistik-tools:calculation-sps": () => this.runCalc("sps"),
      ...
      lumine.commands.add(
        ...
        bySurface({
          "sofistik-tools:open-animator": {
            editor: () => this.editorOpenAnimator(),
            tree: () => this.treeOpenAnimator(),
          },
          "sofistik-tools:open-animator-2018": {
            editor: () => this.editorOpenAnimator({ version: "2018" }),
            tree: () => this.treeOpenAnimator({ version: "2018" }),
          },
          "sofistik-tools:open-report": {
            editor: () => this.editorOpenReport(),
            tree: () => this.treeOpenReport(),
          },
          "sofistik-tools:save-report-as-pdf": {
            editor: () => this.editorOpenReport({ parameters: ["-t", "-printto:PDF"] }),
            tree: () => this.treeOpenReport({ parameters: ["-t", "-printto:PDF"] }),
          },
          "sofistik-tools:save-pictures-as-pdf": {
            editor: () =>
              this.editorOpenReport({ parameters: ["-g", "-picture:all", "-printto:PDF"] }),
            tree: () => this.treeOpenReport({ parameters: ["-g", "-picture:all", "-printto:PDF"] }),
          },
          "sofistik-tools:open-protocol": {
            editor: () => this.editorOpenProtocol(),
            tree: () => this.treeOpenProtocol(),
          },
          "sofistik-tools:open-viewer": {
            editor: () => this.editorOpenViewer(),
            tree: () => this.treeOpenViewer(),
          },
          "sofistik-tools:open-dbinfo": {
            editor: () => this.editorOpenDBNG(),
            tree: () => this.treeOpenDBNG(),
          },
          "sofistik-tools:open-ssd": {
            editor: () => this.editorOpenSSD(),
            tree: () => this.treeOpenSSD(),
          },
          "sofistik-tools:open-wingraf": {
            editor: () => this.editorOpenWinGRAF(),
            tree: () => this.treeOpenWinGRAF(),
          },
          "sofistik-tools:open-result-viewer": {
            editor: () => this.editorOpenResultViewer(),
            tree: () => this.treeOpenResultViewer(),
          },
          "sofistik-tools:open-teddy": {
            editor: () => this.editorOpenTeddy({ parameters: ["-0"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-0"] }),
          },
          "sofistik-tools:open-teddy-single": {
            editor: () => this.editorOpenTeddy({ parameters: ["-nosingle"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-nosingle"] }),
          },
          "sofistik-tools:open-teddy-1": {
            editor: () => this.editorOpenTeddy({ parameters: ["-1"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-1"] }),
          },
          "sofistik-tools:open-teddy-2": {
            editor: () => this.editorOpenTeddy({ parameters: ["-2"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-2"] }),
          },
          "sofistik-tools:open-teddy-3": {
            editor: () => this.editorOpenTeddy({ parameters: ["-3"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-3"] }),
          },
          "sofistik-tools:open-teddy-4": {
            editor: () => this.editorOpenTeddy({ parameters: ["-4"] }),
            tree: () => this.treeOpenTeddy({ parameters: ["-4"] }),
          },
          "sofistik-tools:open-sofiplus": {
            editor: () => this.editorOpenSOFiPLUS(),
            tree: () => this.treeOpenSOFiPLUS(),
          },
          "sofistik-tools:export-cdb": {
            editor: () => this.editorOpenExportCDB(),
            tree: () => this.treeOpenExportCDB(),
          },
          "sofistik-tools:check-version": {
            editor: () => this.editorCheckVersion(),
            tree: () => this.treeCheckVersion(),
          },
        }),
      ),
      lumine.commands.add(
        ...
        withLabels({
          ...
          "sofistik-tools:open-cdbase": () => this.openCHM(),
          "sofistik-tools:open-daten": () => this.openSofistikDaten(),
        }),
      ),
  ...
  // The active editor, but only when it holds SOFiSTiK input. The selector used
  // to be the gate; now that the commands are reachable from the application
  // menu they have to say when they decline, and the handlers that read
  // `editor.getPath()` without a null check need there to be an editor at all.
  sofistikEditor() {
  ...
  getVersion(version, filePath, editor) {
    ...
    return this.environmentProvider.getVersion({
      editor: editor || lumine.workspace.getActiveTextEditor(),
      filePath,
      version,
    });
  },

  // Which language the manuals and examples are wanted in. The environment
  // resolves it from the file's own header or the setting, so the manual a
  // command opens matches the keywords the completion offered.
  getLanguage(filePath, editor) {
    return this.environmentProvider?.getLanguage({
      editor: editor || lumine.workspace.getActiveTextEditor(),
      filePath,
    });
  },
  ...
  getSofPath(version, filePath, editor) {
    ...
    if (!this.environmentProvider) {
      lumine.notifications.addError("The sofistik-environment service is not available");
      return;
    }
    // A version the command was given wins over whatever the file says — the
    // service takes it, so there is no second path-building rule here.
  ...
  runSOFiSTiK(prog, ext, filePath, props, prop2) {
    ...
    if (prop2 && prop2.existsSkip) {
      ...
      lumine.notifications.addWarning(`File doesn't exists "${filePath.replace(/\\/g, "\\\\")}"`);
      return;
    } else {
  ...
  getEditorPath() {
    try {
      return lumine.workspace.getActiveTextEditor().getPath();
    } catch {
      return;
    }
  },
  ...
  editorCheckVersion() {
    ...
    lumine.notifications.addInfo(`SOFiSTiK version: ${version}`);
  },
  ...
  currentHelp(mode, version) {
    ...
    editor.backwardsScanInBufferRange(/^ *[+\-$]?prog +(\w+)/i, range, (object) => {
      found = true;
      // The keyword data keys the modules `WING` and `RESULTS`, but their
      // manuals are named after the programs. Only the file name is aliased;
      // the keyword lookup in `inViewer` still wants what was written.
      const prog = object.match[1].toUpperCase();
      // Lowercase, because that is how every program manual is named on disk.
      // Building the candidate from the uppercased PROG name only ever
      // resolved through the filesystem's case-folding.
      let pro2 = prog.toLowerCase();
      ...
      // The configured language, then the manual published without a suffix,
      // then the other language — a program whose manual exists in one
      // language only still has exactly one manual, and `tunars_0.pdf` is it.
      const filePath = [
        `${pro2}${suffixForLanguage(lang)}.pdf`,
        `${pro2}.pdf`,
        `${pro2}${otherSuffix(lang)}.pdf`,
      ]
    ...
    if (!found) {
      // The command is in the application menu, which carries no dynamic
      // enabled state: it was enabled, it did nothing, and the cursor sitting
      // above every PROG block is not something the user can infer.
  ...
  getViewer(filePath, dest, reuse) {
    ...
    // No service yet — pdf-view is installed but has not handed it over, or is
    // disabled. Its opener matches a `.pdf` URI with a hash on it either way,
    // so the destination still survives the trip through the workspace.
    let hash = "";
    if (dest) {
      hash = `#nameddest=${dest}`;
    }
    lumine.workspace.open(`${filePath}${hash}`, {
      split: "right",
      activatePane: false,
    });
  },
  ...
  inViewer(object, range, filePath, prog, reuse) {
    ...
    if (this.keywordsProvider) {
      ...
      const ctx = this.keywordsProvider.forRelease(resolved?.version, resolved?.language);
