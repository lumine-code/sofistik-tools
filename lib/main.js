...
module.exports = {
  ...
  activate() {
    ...
    // `environmentProvider` is deliberately not reset here: this package
    // provides the environment service, and `provideSofistikEnvironment` can be
    // called before `activate` runs.
  ...
  // The same provider this package hands to other packages, so a command here
  // and a consumer over there can never resolve an installation differently.
  environment() {
    this.environmentProvider ||= new SofistikEnvironmentProvider({
      keywords: () => this.keywordsProvider,
    });
  ...
  getSofPath(version, filePath, editor) {
    ...
    const resolved = this.environment().resolve({
      editor: editor || lumine.workspace.getActiveTextEditor(),
      filePath,
      version,
    });
