...
// SOFiSTiK ships each manual as `<name>_0.pdf` (German) and `<name>_1.pdf`
// (English). A few carry no suffix at all, and a few exist in one language
// only — `tunars_0.pdf` and `thermal_analysis_0.pdf` have no English
// counterpart in any installed version.
const MANUAL = /^(.+?)(_[01])?\.pdf$/i;
const SUFFIX = { English: "_1", German: "_0" };
...
module.exports = class HelpList {
  ...
  async update() {
    ...
    const wanted = SUFFIX[lang] ?? SUFFIX.English;
